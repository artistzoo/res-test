$(function(){
  ...
  //mobile navigation__________________________
  ...
  //PC navigation
  /* 메인메뉴에 호버를 하면 서브가 나아고 이 상태에서 검색버튼을 누르면 검색창이 나오고, 검색창이 나왔을 때는 메뉴에 호버했을 때 서브메뉴가 안나오게 함 */
  ...
  hNavBtn.mouseenter(hNav); //메인메뉴 오버시 서브메뉴 전체가 나오게 하는 함수 호출
  subNav.mouseleave(hNav_reset); //서브메뉴 전체영역에 나갔을 때 다시 올라가는 함수 호출
  ...
  function hNav(){
    if(!schWrapIs && !hNavIs){ //검색창과 서브메뉴가 나와있지 않았을 때
      ...
      hNavIs = true; //서브메뉴가 나와있는 상테에서는 truefh qusrud
    };
  };
  ...
  schBtn.click(function(){
    if (!schWrapIs){ //false라면 (schWrapIs나오지 않았으면)
      sch();
    }else{ //나왔을 때
      sch_reset();
    }
