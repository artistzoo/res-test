$(function(){
  ...
  const preBtn = $('.s3_slide .arrow.left');

  //좌우버튼이 2개기 때문에 row1, row2에 있는 버튼인지 알아내야 함
  nextBtn.on('click', function(e){
    ...
    if(slider.hasClass('row1')){
      //hasClass(); 클래스를 가지고 있는지 확인하는 메서드
      /* console.log(0); */
      next(0);
...
function next(i){
  ...
  let s1Length = crt.children('img').length; //이미지 개수(위아래 개수가 다름)

  /* console.log(index, s1Length); */
  index++;
  if(index==s1Length){
    index=0;
  };
...
function prev(i){
  ...
  let s1Length = crt.children('img').length;
  /* console.log(index, s1Length); */
  index--;
  if(index<0){
    index=s1Length-1;
